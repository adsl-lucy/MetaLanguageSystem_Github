#include "AudioObjWgt.h"
#pragma execution_character_set("utf-8")

AudioObjWgt::AudioObjWgt(QWidget* parent)
	: QWidget(parent)
{
	ui.setupUi(this);
	init();
...
AudioObjWgt::~AudioObjWgt()
{}

void AudioObjWgt::initUI()
{
	connect(ui.btn_updateTimeSecCht, &QPushButton::clicked, this, &AudioObjWgt::slot_updateTimeSecChtBtnClicked);
	connect(ui.btn_updateHotCht, &QPushButton::clicked, this, &AudioObjWgt::slot_updateHotChtBtnClicked);
	connect(ui.btn_AudioObj_GenResult, &QPushButton::clicked, this, &AudioObjWgt::slot_GenAudioObjDataBtnClicked);

	connect(ui.HSlider_TimeFrameIndex, &QAbstractSlider::valueChanged, this, &AudioObjWgt::slot_TimeFrameSliderMoved);

	connect(ui.btn_play, &QPushButton::clicked, this, &AudioObjWgt::slot_play);
	connect(ui.btn_jump, &QPushButton::clicked, this, &AudioObjWgt::slot_jump);
	connect(ui.btn_stop, &QPushButton::clicked, this, &AudioObjWgt::slot_stop);

	connect(ui.btn_lastFrame, &QPushButton::clicked, this, &AudioObjWgt::slot_btnLastFrameClicked);
	connect(ui.btn_nextFrame, &QPushButton::clicked, this, &AudioObjWgt::slot_btnNextFrameClicked);

	connect(DataCenterManager::instance(), &DataCenterManager::sgn_STFTDataSaved, this, &AudioObjWgt::slot_getNewSTFTData);
	//connect(DataCenterManager::instance(), &DataCenterManager::sgn_DiffDataSaved, this, &AudioObjWgt::slotDiffResultGet);
	...
	//ui.comboBox_TimeSecChtBarType->setItemData(0, QVariant((int)EN_AudioObj_TimeSecChtBarType::STFT));
	//ui.comboBox_TimeSecChtBarType->setItemData(1, QVariant((int)EN_AudioObj_TimeSecChtBarType::EffectiveOscillation));
	//ui.comboBox_TimeSecChtBarType->setItemData(2, QVariant((int)EN_AudioObj_TimeSecChtBarType::BDSNR));
	//ui.comboBox_TimeSecChtBarType->setItemData(3, QVariant((int)EN_AudioObj_TimeSecChtBarType::DiffGroup));
...
void AudioObjWgt::initUpdateTimer()
{
	...
	connect(m_pUpdateSectionTimer, &QTimer::timeout, this, &AudioObjWgt::slot_updateTimerTimeUp);

}

void AudioObjWgt::init()
{
	...
	connect(m_pSTFTHotCht, &FFT_Chart_STFTHotGram::sgn_HotRectGenerateState, this, &AudioObjWgt::slot_updateHotChtGenState);


	//m_pAuObjTimeSecCht = new TimeSecCht_AudioObj(this);
	//m_pAuObjTimeSecCht->setTitleText(QString("Diff Relevance TimeSec Cht"));
	//ui.HLayout_TimeSecCht->insertWidget(0, m_pAuObjTimeSecCht);
	//connect(m_pAuObjTimeSecCht, &TimeSecCht_AudioObj::sgn_SecRectGenerateState, this, &AudioObjWgt::slot_updateTimeSecGenState);
}

void AudioObjWgt::updateSTFTLabel(const ST_STFTOutputData& data)
{
...
void AudioObjWgt::updateSTFTPanel(const ST_STFTOutputData& data)
{
...
void AudioObjWgt::updateTimeFrameLabel(QTime FrameStartTime, QTime FrameMidTime, QTime FrameEndTime)
{
...
void AudioObjWgt::slot_TimeFrameSliderMoved(int newVal)
{
	...
	//m_pAuObjTimeSecCht->setCurrentFrameIndex(newVal);
...
void AudioObjWgt::slot_updateTimerTimeUp()
{
...
void AudioObjWgt::slot_updateHotChtGenState(int percentage, int activateThreadCount, QString errstr)
{
...
void AudioObjWgt::slot_updateTimeSecGenState(int percentage, int activateThreadCount, QString errstr)
{
...
void AudioObjWgt::slot_getAudioObjectMsg(int percentage, int activateThreadCount, QString errstr)
{
	ui.progressBar_AudioObj_GenResult->setValue(percentage);
	if (percentage != 100)
	{
		ui.label_AudioObj_GenState->setText(QString("活跃线程：%1 %2").arg(activateThreadCount).arg(errstr));
	}
	else
	{
		ui.label_AudioObj_GenState->setText(QString("生成结束"));
	}
}

void AudioObjWgt::slot_AudioObjecResultGet(QString DataKey)
{
...
void AudioObjWgt::slot_btnNextFrameClicked()
{
...
void AudioObjWgt::slot_btnLastFrameClicked()
{
...
void AudioObjWgt::slot_TimeSecChtTypeChanged()
{
	//EN_AudioObj_TimeSecChtBarType currentType = (EN_AudioObj_TimeSecChtBarType)ui.comboBox_TimeSecChtBarType->currentData().toInt();
	//m_pAuObjTimeSecCht->setChtType(currentType);
}

void AudioObjWgt::slot_getNewSTFTData(QString dataKey)
{
	...
	//m_pAuObjTimeSecCht->setSTFTResultPara(data.InputPara);
	//m_pAuObjTimeSecCht->setCurrentFrameIndex(0);
...
void AudioObjWgt::slot_AutoGenAudioObjInputData(QString dataKey)
{
	ui.tabWidget_DiffNetPannel->setCurrentIndex(0);
	slot_GenAudioObjDataBtnClicked();
}

void AudioObjWgt::slot_updateHotChtBtnClicked()
{
...
void AudioObjWgt::slot_GenAudioObjDataBtnClicked()
{
	/*ST_DiffNetPara para;
...
void AudioObjWgt::slot_updateTimeSecChtBtnClicked()
{
	...
	//m_pAuObjTimeSecCht->setDrawFreqRanged(showMinFreq, showMaxFreq);
	//m_pAuObjTimeSecCht->setDrawDBRange(dbMinVal, dbMaxVal);
	//m_pAuObjTimeSecCht->resetFrameRect(m_STFTDataKey);

}

void AudioObjWgt::slot_play()
{
