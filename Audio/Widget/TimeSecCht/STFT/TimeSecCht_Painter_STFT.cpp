#include "TimeSecCht_Painter_STFT.h"

TimeSecCht_Painter_STFT::TimeSecCht_Painter_STFT(QObject* parent)
	:TimeSecCht_Painter(parent)
{
	m_pSTFTwatcher = new QFutureWatcher<void>(this);
	connect(m_pSTFTwatcher, &QFutureWatcher<void>::progressValueChanged, this, &TimeSecCht_Painter_STFT::slot_GenProgressValueChanged);
}

void TimeSecCht_Painter_STFT::resetTimeSecChtRect(const QString dataKey, const ST_STFTInputpara& para)
{
	...
	{
		qDebug() << QString("TimeSecCht_Painter_STFT::resetFrameRect STFTMapPos end! Key : %1").arg(dataKey);
	...
	updateTitleText(EN_TimeSecCht_PaintType::STFT);
	...
	QFuture<void> future = QtConcurrent::map(m_currentProcessVec, TimeSecCht_Painter_STFT::FillTargetFrame);
	m_pSTFTwatcher->setFuture(future);
	m_nLastReportPercantage = -1;
	slot_GenProgressValueChanged(0);
}

void TimeSecCht_Painter_STFT::drawValueArea(QPainter& painter, QRect& drawArea)
{
	...
	const QVector<ST_FFTFreqBar>& resultVec = m_vecFrameBar_STFT[m_nCurrentFrameIndex];
	...
	painter.setBrush(TimeSecCht_Audio::const_Cr_STFTOnlyBar);
...
void TimeSecCht_Painter_STFT::drawMousePosValue(QPainter& painter, QRect& drawArea)
{
	QPen RectPen;
	RectPen.setWidth(2);
	RectPen.setColor(Qt::black);
...
void TimeSecCht_Painter_STFT::drawLengend(QPainter& painter, QRect& RightColorMapRect)
{
	QPen NormalTextPen;
	...
	const QString STFTLegendStr = QString("STFT Val");
	drawCrRectLegend(painter, RightColorMapRect, 1, STFTLegendStr, TimeSecCht_Audio::const_Cr_STFTOnlyBar);
}

bool TimeSecCht_Painter_STFT::generateFreqBarVec(const int STFTFrameIndex, const ST_FFTOutputData& frameData)
{
...
void TimeSecCht_Painter_STFT::slot_GenFinished()
{
	emit  sgn_GenFinished();
	m_currentProcessVec.clear();
}

void TimeSecCht_Painter_STFT::slot_GenProgressValueChanged(int val)
{
	if (m_currentProcessVec.isEmpty())
	{
		qDebug() << "TimeSecCht_Painter_STFT::slot_GenProgressValueChanged RectVec empty!";
		return;
	}
